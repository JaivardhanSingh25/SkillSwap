    ...
    <div className="bg-white rounded-xl shadow-md border hover:shadow-lg transition-shadow duration-300 overflow-hidden">
      ...
      <div className="bg-linear-to-r from-gray-50 to-gray-100 p-6 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-xl font-bold text-gray-800 truncate">{user.name || "Unknown"}</h3>
          <p className="text-gray-600 text-sm">{user.age ? `${user.age} years old` : "Age not available"}</p>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-gray-600">
          <span className="text-sm">📍 {user.location || "Location unknown"}</span>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {(user.skillWanted || []).map((skill, idx) => (
              <span key={idx} className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">
                {skill}
              </span>
            ))}
